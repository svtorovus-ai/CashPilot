...
public class SettingsActivity extends Activity {
    ...
    private CharSequence functionInfo() {
        ...
        appendInfo(out, "КНОПКИ НА ГОЛОВНОМУ ЕКРАНІ\n", 0xFFB8D7FF, true);
        appendInfo(out, "⚙ Налаштування — відкриває цей екран для зміни підключення, ставок, URL веб-статистики та перевірки оновлень.\n", 0xFFE8EEF5, false);
        appendInfo(out, "‹ / › Місяці — відкривають попередній або наступний місяць; те саме можна зробити свайпом.\n", 0xFFE8EEF5, false);
        appendInfo(out, "↻ Оновити статистику — повністю скидає локальні зміни вибраного місяця й повторно завантажує його з telemetry-сервера.\n", 0xFFE8EEF5, false);
        appendInfo(out, "ⓘ Інформація — відкриває довідник функцій, статусів, розрахунку та синхронізації.\n\n", 0xFFE8EEF5, false);
        ...
        appendInfo(out, "CashPilot автоматично перевіряє нові версії під час запуску та повернення в додаток. Кнопка «Перевірити оновлення CashPilot» запускає перевірку негайно. Якщо нова версія знайдена, додаток завантажує підписаний APK, перевіряє його контрольну суму й відкриває системне встановлення Android. Потрібно лише підтвердити інсталяцію; без підтвердження Android не дозволяє сторонньому додатку встановити оновлення непомітно. Оновлення не повинно стирати налаштування, кеш або локальні статуси.\n", 0xFFE8EEF5, false);
        return out;
    }
